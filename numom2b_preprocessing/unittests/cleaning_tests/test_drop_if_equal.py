...
class CleanDropIfEqualTests(unittest.TestCase):
    ...
    def test_drop_if_equal_2():
        ...
        _cleanings = [{"columns": ["a"], "drop_if": ["==", 1.0]}]
        _expected = DataFrame(
            {
                "a": [2.0, 3.0],
                "b": [3.0, 4.0],
    ...
    def test_drop_if_equal_3():
        ...
        _cleanings = [{"columns": ["b"], "drop_if": ["==", 1.0]}]

        _table = preprocess._clean_variables(_table_3, _cleanings)

        assert_frame_equal(_table_3, _table)
    ...
    def test_drop_if_equal_4():
        ...
        _cleanings = [{"columns": ["a"], "drop_if": ["==", "B"]}]
        _expected = DataFrame(
            {
                "a": ["A"],
                "b": ["C"],
    ...
    def test_drop_if_equal_6(self):
        ...
        with self.assertRaises(Exception):
            _table = preprocess._clean_variables(_table_6, _cleanings)
